...
public class Boid : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Restent éloignés du sol
        float distSol = Mathf.Max(0, transform.position.y - Sol);
        if (distSol < 5)
        {
            float forceRejet = Mathf.Pow(1.0f - (distSol / 5), 2) * forceDecolle;
            sumForces += new Vector3(0, forceDecolle, 0);
        }
